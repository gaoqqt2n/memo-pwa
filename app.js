...
// ====== Cloud / Local 切替 ======
function fetchAndRenderFromCloud(){ if(!currentUser) return Promise.resolve(); return fetchMemosCloud(currentUser.id).then(function(res){ var data=res&&res.data; var error=res&&res.error; if(error){ console.error(error); return; } memos=(data||[]).map(function(row){ return { id:row.id, title: row.title || (row.content?(row.content.split('')[0].slice(0,30)||'無題'):'無題'), content: row.content||'', color: row.color||'gray', fontSize: row.font_size||16, createdAt: row.created_at ? new Date(row.created_at).getTime() : Date.now(), updatedAt: row.updated_at ? new Date(row.updated_at).getTime() : Date.now(), position: row.position||0 }; }); renderList(); }).catch(function(e){ console.error(e); }); }
